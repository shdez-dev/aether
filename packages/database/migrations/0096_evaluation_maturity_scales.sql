ALTER TABLE evaluation_standards
  ADD COLUMN maturity_levels JSONB NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE initiative_evaluations
  ADD COLUMN maturity JSONB;
...
BEGIN
  ...
  AND (
    ...
    OR NEW.maturity_levels IS DISTINCT FROM OLD.maturity_levels
...
DROP TRIGGER evaluation_standards_preserve_applied_versions
  ON evaluation_standards;

CREATE TRIGGER evaluation_standards_preserve_applied_versions
BEFORE UPDATE OF organization_id, name, version, criteria, maturity_levels,
