...
export default function AetherPage() {
  ...
  async function acceptInvitation(event: FormEvent) {
    ...
    try {
      ...
      const invitation = (await response.json()) as { organizationId: string };
      await loadOrganizations();
      setOrganizationId(invitation.organizationId);
      setInvitationToken("");
      setMessage("Invitación aceptada. Ya puedes entrar a la organización.");
      setError("");
    } catch (caught) {
      setError(
        caught instanceof Error
  ...
  if (authState === "anonymous")
    return (
      <main className="auth-shell">
        <Card title="Tu espacio de trabajo">
          <p>Inicia sesión para acceder a tus iniciativas y proyectos.</p>
          <a className="login-link" href="/auth/login">
            Iniciar sesión
          </a>
          <p>
            <a href="/">Volver a Aether</a>
          </p>
        </Card>
      </main>
    );
      ...
      <main className="workspace-main" id="content">
        <header className="workspace-topbar">
          ...
          <div className="workspace-topbar__controls">
            ...
            <button
              className="workspace-topbar__refresh"
              type="button"
              title="Actualizar el espacio"
              aria-label="Actualizar el espacio"
        ...
        <div className="workspace-main__content">
          ...
          {activeView === "initiatives" ? (
            <>
              <div className="workspace-section-heading">
                <div>
                  <p className="workspace-kicker">DEL CONTEXTO A LA DECISIÓN</p>
                  <h1>Iniciativas</h1>
                  <p>
                    Presenta, evalúa y decide sin perder el hilo de cada idea.
                  </p>
                </div>
                <button
                  type="button"
                          ...
                          <section className="evidence-card">
                            ...
                            {evaluation.maturity ? (
                              <p>
                                ...
                                {evaluation.maturity.minimumQualityPercentage}%
                                de calidad)
                              </p>
                            ) : null}
                          </section>
          ...
          {activeView === "projects" ? (
            <>
              <div className="workspace-section-heading">
                <div>
                  <p className="workspace-kicker">DE LA DECISIÓN A LA ACCIÓN</p>
                  <h1>Proyectos</h1>
                  <p>Avanza con claridad sobre hitos, responsables y tareas.</p>
                </div>
              </div>
              <section
                id="projects"
                    ...
                    <div className="initiative-list">
                      {projects.map((project) => (
                        ...
                        >
                          ...
                          <Status>{projectStatusLabel(project.status)}</Status>
                        </button>
                      ))}
                    </div>
                  ) : (
                      ...
                      <>
                        ...
                        >
                          ...
                          {baselineDifference?.baseline ? (
                            <>
                              <p className="muted">
                                Versión {baselineDifference.baseline.version}{" "}
                                aprobada el{" "}
                                {new Date(
          ...
          {activeView === "organization" ? (
            <>
              <div className="workspace-section-heading">
                <div>
                  <p className="workspace-kicker">PERSONAS Y CONTEXTO</p>
                  <h1>Organización</h1>
                  <p>
                    Prepara espacios, invita colaboradores y gestiona tu
                    participación.
                  </p>
                </div>
              </div>
              ...
              >
                ...
                <form className="ui-card" onSubmit={createWorkspace}>
                  ...
                  <Button
                    type="submit"
                    disabled={
                      !organizationId || !capabilities.canCreateWorkspace
                    }
                  >
                    Crear workspace
                  </Button>
                ...
                <form className="ui-card" onSubmit={acceptInvitation}>
                  <h2>Unirme a otra organización</h2>
                  <Field
