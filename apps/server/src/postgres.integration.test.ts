...
describe("PostgreSQL integration", () => {
  it("renews, expires and revokes opaque sessions with append-only audit", async () => {
    ...
    await expect(
      pool.query("DELETE FROM auth_session_audit_events WHERE actor_id = $1", [
        actorId,
      ]),
    ).rejects.toThrow("append-only");
  }, 120_000);

  it("persists and replays idempotent mutation responses", async () => {
    ...
    await expect(store.reserve(pending)).resolves.toEqual({ kind: "claimed" });
  });

  it("isolates tenants and persists project plus outbox event atomically", async () => {
